...
3. Cepstral domain: taking a Fourier-flavored transformation one step further, we can also analyze what frequencies are present in the spectrum (used for pitch tracking in `analyze()`) or spectrogram (function `modulationSpectrum()` - more detail in section 7). 
...
4. Other. Less common visual representations of audio include neurologically inspired spectro-temporal receptive fields, gammatones, etc. For example, soundgen can plot self-similarity matrices that represent spectral changes over time (more detail in section 8):
...
* `peakFreq`: the frequency with maximum spectral energy (Hz)
