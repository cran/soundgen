# TODO: getSurprisal - try different time scales, DTW instead of autocorrelation; make color theme in apps compatible with dark mode; spectrogram etc - relative paths not working on Windows only, abs path only if saveAudio is the same as input folder (?); checkInput complains when running any folder function on some Win10 machines (?)
...
soundgen = function(
  ...
  if (!is.na(saveAudio)) {
    ...
    writeAudio(bout, audio = audio, filename = saveAudio)
